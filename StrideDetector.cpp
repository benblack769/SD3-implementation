...
{
  if(myLostPoint == true){
    // We assume that the lost points were handled before the next access was processed.
    clearLostPoints();
  }
  ...
  if(change < 0){
    ...
    if(myState == FirstObserved){
      if(myLostPoint == true){
	// This occurs when we had two memory accesses but the third memory access
	// didn't match the stride of the first two accesses.  So, we're backing
	// up, but we want to make sure that we don't lose those two preceding accesses
	// that were never previously viewed as part of a stride.
	return LOSTPOINT;
      }
      else{
	// This doesn't match old stride but it could be start of new stride
	// The only way for this to occur is if you were backing up from StrideLearned
	// and this had previously been recognized as a stride
	return UNKNOWN;
      }
    }
    else{
	...
	if(myLastAccessUnknown == true){
	  // Need to recognize there were 3 accesses to this address that haven't
	  // been tracked yet.
	  myLastAccessUnknown = false;  // clear boolean so next access is counted singularly
	  return NEWPOINT;
	}
	else{
	  return POINT;
