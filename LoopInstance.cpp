    ...
    for (iter = rangeIter.first; iter != rangeIter.second; iter++) {
        if (readOrWrite == iter->second.mode) {
            ...
            if (result == POINT) {
                ...
                if (lastType == UNKNOWN) {
                    // If the previous type was UNKNOWN, that means there was an
                    // access to this memory address that was neither classified
                ...
                // determination yet perhaps because it is the first element.
            }
        }
