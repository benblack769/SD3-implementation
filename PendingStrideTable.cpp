...
void PendingStrideTable::addNewStride(long long PC, long long lowAddress,
                                      long long highAddress,
                                      long long strideLength, int numAccesses,
                                      long long accessSize,
                                      MemAccessMode readOrWrite) {
    StrideEntry temp;
    temp.stride = new Stride(lowAddress, highAddress, strideLength, numAccesses,
                             accessSize, readOrWrite);
    temp.killed = false;

    myPendingStrides.insert(pair<long long, StrideEntry>(PC, temp));
}

bool PendingStrideTable::doesStrideExist(long long PC, long long memoryAddr,
                                         long long strideLength,
                                         MemAccessMode readOrWrite) {
    pair<multimap<long long, StrideEntry>::iterator,
         multimap<long long, StrideEntry>::iterator>
        rangeIter;
    ...
    for (iter = rangeIter.first; iter != rangeIter.second; iter++) {
        ...
        if (readOrWrite == iter->second.stride->getAccessMode() &&
            strideLength == iter->second.stride->getStride()) {
...
bool PendingStrideTable::updateExistingStride(long long PC,
                                              long long memoryAddr,
                                              long long strideLength,
                                              MemAccessMode readOrWrite) {
    pair<multimap<long long, StrideEntry>::iterator,
         multimap<long long, StrideEntry>::iterator>
...
bool PendingStrideTable::killStride(long long PC, long long memoryAddr,
                                    long long strideLength,
                                    MemAccessMode readOrWrite) {
    pair<multimap<long long, StrideEntry>::iterator,
         multimap<long long, StrideEntry>::iterator>
...
void PendingStrideTable::print() {
    ...
    for (iter = myPendingStrides.begin(); iter != myPendingStrides.end();
         iter++) {
        cout << "PC: " << iter->first;
