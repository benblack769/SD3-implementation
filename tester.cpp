...
void handleStrideDetectorResult(long long pc, long long memAddr, MemAccessType result, StrideDetector *strDet)
{
  ...
  if(result == UNKNOWN){
    ...
    // This occurs when a stride hasn't actually been learned yet so the last two
    // accesses need to be accounted for.
    ...
    // This happens when there have been 3 consecutive accesses to this point (stride==0)
    // We need to account for all three accesses when adding the point
    ...
    // This would occur if the current access did not meet the current stride, making the
    // StrideDetector go back from a state where the stride was learned.
    if(pendingPointTable->doesPointExist(memAddr, pc, READ)){
      pendingPointTable->updateExistingPoint(memAddr, pc, READ);
    }
    else{
    ...
    if(pendingStrideTable->doesStrideExist(pc, memAddr, strideLength, READ)){  // just hard coding R/W for now
      pendingStrideTable->updateExistingStride(pc, memAddr, strideLength, READ);
    }
    else{
      pendingStrideTable->addNewStride(pc, strDet->getFirstMemAddr(), strDet->getPrevMemAddr(), strideLength, 3, 4, READ);  // hard coding R/W and access Size for testing
...
/* This should be called after all inputs have been processed.  It goes through all of the
 * StrideDetectors and sees if they are in any state that might indicate that a previous
 * access has not been propagated either to the PendingStrideTable or PendingPointTable.
 * Since anything classified as a Stride is automatically put into the PendingStrideTable,
 * anything left in a Stride Detector will be viewed as a Point and will be added to the
 * PendingPointTable.
 */
