...
void handleStrideDetectorResult(long long pc, long long memAddr, MemAccessType result, StrideDetector *strDet) {
    ...
    if (result == UNKNOWN) {
        ...
        if (result == UNKNOWN) {
            ...
            if (pendingStrideTable->doesStrideExist(pc, memAddr, strideLength,
                ...
                pendingStrideTable->addNewStride(pc, strDet->getFirstMemAddr(), strDet->getPrevMemAddr(), strideLength,
                                                 3, 4,
    ...
    void purgeStrideDetectorState() {
         ...
         * Stores the current phase of the FSM and passes it as an argument for the recursive call of the PointOrStride
         * method. Prints out the pending tables for both point addresses and stride addresses.
         */
